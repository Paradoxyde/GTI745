...
class Palette {
	...
	public Palette() {
		...
		b = new PaletteButton( 7*W, 0, "Ver. Flip", "", true );
		...
		b = new PaletteButton( 5*W, 0, "R : 0.0", "", false );
                b.height = H * 2 / 3;
                b.width = W * 2;
		customColorRed_buttonIndex = buttons.size();
		buttons.add( b );
                
		b = new PaletteButton( 5*W, H * 2 / 3, "G : 0.0", "", false );
                b.height = H * 2 / 3;
                b.width = W * 2;
		customColorGreen_buttonIndex = buttons.size();
		buttons.add( b );
                
		b = new PaletteButton( 5*W, H * 4 / 3, "B : 0.0", "", false );
                b.height = H * 2 / 3;
                b.width = W * 2;
			...
			if ( palette.contains( x, y ) ) {
				...
				if ( cursorContainer.getNumCursors() == 0 ) {
					...
					if (
						...
						indexOfButton == palette.customColorRed_buttonIndex
						|| indexOfButton == palette.customColorGreen_buttonIndex
						|| indexOfButton == palette.customColorBlue_buttonIndex
					) {
						// We transition to the color corresponding to the button
						palette.buttons.get( palette.currentlyActiveColorButton ).isPressed = false;
						palette.currentlyActiveColorButton = indexOfButton;
						...
						if ( indexOfButton == palette.customColorRed_buttonIndex ) {
							palette.current_red = (x - button.x0 - palette.x0) / button.width;
                                                        if (palette.current_red < 0) palette.current_red = 0;
                                                        if (palette.current_red > 1.0f) palette.current_red = 1.0f;
                                                        button.label = "R : " + (int)(palette.current_red * 100) + "%";
						}
						else if ( indexOfButton == palette.customColorGreen_buttonIndex ) {
							palette.current_green = (x - button.x0 - palette.x0) / button.width;
                                                        if (palette.current_green < 0) palette.current_green = 0;
                                                        if (palette.current_green > 1.0f) palette.current_green = 1.0f;
                                                        button.label = "G : " + (int)(palette.current_green * 100) + "%";
						}
						else if ( indexOfButton == palette.customColorBlue_buttonIndex ) {
							palette.current_blue = (x - button.x0 - palette.x0) / button.width;
                                                        if (palette.current_blue < 0) palette.current_blue = 0;
                                                        if (palette.current_blue > 1.0f) palette.current_blue = 1.0f;
                                                        button.label = "B : " + (int)(palette.current_blue * 100) + "%";
						}
			...
			if ( cursor.type == MyCursor.TYPE_NOTHING ) {
				...
				if ( type == MultitouchFramework.TOUCH_EVENT_UP ) {
					...
					if ( cursor.indexOfButton == palette.movePalette_buttonIndex ) {
						...
						cursor.indexOfButton == palette.customColorRed_buttonIndex
						|| cursor.indexOfButton == palette.customColorGreen_buttonIndex
						|| cursor.indexOfButton == palette.customColorBlue_buttonIndex
					) {
						// We transition to the color corresponding to the button
						palette.buttons.get( palette.currentlyActiveColorButton ).isPressed = false;
						palette.currentlyActiveColorButton = cursor.indexOfButton;
                                                
                                                PaletteButton button = palette.buttons.get( cursor.indexOfButton );
						button.isPressed = true;

						if ( cursor.indexOfButton == palette.customColorRed_buttonIndex ) {
							palette.current_red = (x - button.x0 - palette.x0) / button.width;
                                                        if (palette.current_red < 0) palette.current_red = 0;
                                                        if (palette.current_red > 1.0f) palette.current_red = 1.0f;
                                                        button.label = "R : " + (int)(palette.current_red * 100) + "%";
						}
						else if ( cursor.indexOfButton == palette.customColorGreen_buttonIndex ) {
							palette.current_green = (x - button.x0 - palette.x0) / button.width;
                                                        if (palette.current_green < 0) palette.current_green = 0;
                                                        if (palette.current_green > 1.0f) palette.current_green = 1.0f;
                                                        button.label = "G : " + (int)(palette.current_green * 100) + "%";
						}
						else if ( cursor.indexOfButton == palette.customColorBlue_buttonIndex ) {
							palette.current_blue = (x - button.x0 - palette.x0) / button.width;
                                                        if (palette.current_blue < 0) palette.current_blue = 0;
                                                        if (palette.current_blue > 1.0f) palette.current_blue = 1.0f;
                                                        button.label = "B : " + (int)(palette.current_blue * 100) + "%";
		...
		if ( Constant.NUM_USERS == 1 ) {
			...
			for ( int j = 0; j < Constant.NUM_USERS; ++j ) {
				...
				userContexts[j].setPositionOfCenterOfPalette(
					Constant.INITIAL_WINDOW_WIDTH / 2 + (float)(radius*Math.cos(angleInRadians + Math.PI / 2)),
					Constant.INITIAL_WINDOW_HEIGHT / 2 + (float)(radius*Math.sin(angleInRadians + Math.PI / 2))
				);
			}
		}
